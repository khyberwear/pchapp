...
export function getCurrentViewTransition(scope = document) {
    return getScopeData(scope).currentViewTransition;
}
function getScopeData(scope = document) {
...
if (document.startViewTransition) {
    ...
    document.body.insertAdjacentHTML('beforeend', "<style class='vtbag-ud-feature-detection'>* {view-transition-name: none !important}</style>");
    try {
        document
...
export function mayStartViewTransition(param, ext = {}) {
    let { scope = document, collisionBehavior = 'skipOld', speedUpWhenChained = 1, respectReducedMotion = true, useTypesPolyfill = 'never', catchErrors = true, } = ext;
    if (!('documentElement' in scope) && nativeSupport !== 'scoped') {
        console.warn(`Using a scope other than document is only supported in browsers with scoped view transitions`);
        scope = document;
    ...
    let surrogate = false;
    if ((collisionBehavior === 'skipNew' && scopeData.currentViewTransition) ||
        collisionBehavior === 'never' ||
        nativeSupport === 'none' ||
        reduceMotion) {
        surrogate = true;
    ...
    if (!scopeData.currentViewTransition || collisionBehavior === 'skipOld') {
        ...
        scopeData.currentViewTransition = polyfilledTypes(scope, (scopeData.currentViewTransition = surrogate
            ? createViewTransitionSurrogate(unchainUpdates)
            : scope.startViewTransition(unchainUpdates)), useTypesPolyfill === 'always' || (useTypesPolyfill !== 'never' && nativeSupport === 'partial'));
        if (catchErrors) {
    ...
    async function unchainUpdates() {
        const current = scopeData.updates.splice(0, scopeData.updates.length - (scopeData.keepLast ? 1 : 0));
