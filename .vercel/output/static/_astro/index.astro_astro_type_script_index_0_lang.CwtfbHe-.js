import{$ as b}from"./authStore.BgXoZ_hx.js";import"./index.daetGfa_.js";let o=[],i="all";const d=document.getElementById("orders-table-body"),f=document.getElementById("order-modal"),c=document.getElementById("modal-backdrop"),g=document.getElementById("modal-title"),m=document.getElementById("modal-content");document.addEventListener("DOMContentLoaded",()=>{const t=b.get();!t||t.role!=="admin"?window.location.href="/":(u(),v())});function v(){document.getElementById("refresh-orders")?.addEventListener("click",u),document.querySelectorAll(".filter-tab").forEach(t=>{t.addEventListener("click",e=>{const a=e.target.dataset.filter||"all";y(a)})}),document.getElementById("hs-table-search")?.addEventListener("input",t=>{const e=t.target.value.toLowerCase();p(e)}),document.getElementById("close-modal-x")?.addEventListener("click",l),document.getElementById("close-modal-btn")?.addEventListener("click",l),c?.addEventListener("click",l),d?.addEventListener("click",async t=>{const r=t.target.closest("button[data-action]");if(!r)return;const a=r.dataset.order,s=r.dataset.action;!a||!s||(s==="view"?C(a):await E(a,s))})}function y(t){i=t,document.querySelectorAll(".filter-tab").forEach(r=>{r.classList.remove("active","border-orange-500","text-orange-600","dark:text-orange-400"),r.classList.add("border-transparent","text-gray-500","dark:text-neutral-400")});const e=document.querySelector(`[data-filter="${t}"]`);e&&(e.classList.add("active","border-orange-500","text-orange-600","dark:text-orange-400"),e.classList.remove("border-transparent","text-gray-500","dark:text-neutral-400")),p()}function p(t=""){let e=o;i!=="all"&&(e=e.filter(r=>r.status.toLowerCase()===i)),t&&(e=e.filter(r=>r.order_number.toLowerCase().includes(t)||r.customer_name.toLowerCase().includes(t)||r.customer_email.toLowerCase().includes(t))),L(e)}function k(){const t={pending:o.filter(n=>n.status.toLowerCase()==="pending").length,processing:o.filter(n=>n.status.toLowerCase()==="processing").length,dispatched:o.filter(n=>n.status.toLowerCase()==="dispatched").length,completed:o.filter(n=>n.status.toLowerCase()==="completed").length},e=document.getElementById("stat-pending"),r=document.getElementById("stat-processing"),a=document.getElementById("stat-dispatched"),s=document.getElementById("stat-completed");e&&(e.textContent=t.pending.toString()),r&&(r.textContent=t.processing.toString()),a&&(a.textContent=t.dispatched.toString()),s&&(s.textContent=t.completed.toString())}function h(t){const e=t.toLowerCase(),r={pending:"bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400",processing:"bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400",dispatched:"bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400",completed:"bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400",cancelled:"bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400"};return r[e]||r.pending}function w(t){const e=t.status.toLowerCase(),r=[];return e==="pending"&&r.push(`<button data-order="${t.order_number}" data-action="processing" class="inline-flex items-center gap-1 rounded-lg bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-600 hover:bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400 dark:hover:bg-blue-900/50">
        ...
        Cancel
      </button>`),r.join("")}function L(t){if(d){if(t.length===0){d.innerHTML=`
        <tr>
      ...
      <tr class="hover:bg-gray-50 dark:hover:bg-neutral-700/50 transition-colors">
        ...
        <td class="px-6 py-4 whitespace-nowrap">
          <span class="inline-flex items-center gap-x-1.5 py-1.5 px-3 rounded-full text-xs font-medium ${h(e.status)}">
    ...
    `).join("")}}async function u(){if(d)try{d.innerHTML=`
      ...
      `;const t=await fetch("/api/admin/orders"),e=await t.json();if(!t.ok)throw new Error(e.error||"Failed to fetch orders");o=e.orders||[],k(),p()}catch(t){console.error("Error loading orders:",t),d.innerHTML=`
        <tr>
          <td colspan="6" class="px-6 py-12 text-center">
            <div class="flex flex-col items-center">
        ...
        <div class="space-y-6">
          <div class="grid grid-cols-2 gap-4">
            <div class="rounded-lg bg-gray-50 p-4 dark:bg-neutral-700">
              ...
              <span class="mt-1 inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${h(e.status)}">
                ${e.status.charAt(0).toUpperCase()+e.status.slice(1)}
              </span>
            </div>
            <div class="rounded-lg bg-gray-50 p-4 dark:bg-neutral-700">
      ...
      `),f?.classList.remove("hidden"),c?.classList.remove("hidden"),document.body.style.overflow="hidden"}function l(){f?.classList.add("hidden"),c?.classList.add("hidden"),document.body.style.overflow=""}const x=localStorage.getItem("user"),$=document.getElementById("auth-loading"),M=document.getElementById("admin-content");x?JSON.parse(x).role!=="admin"?window.location.href="/":($?.classList.add("hidden"),M?.classList.remove("hidden")):window.location.href="/login";const B=document.getElementById("logout-btn");B?.addEventListener("click",()=>{localStorage.removeItem("user"),window.location.href="/login"});
